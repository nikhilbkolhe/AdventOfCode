...
for (let i = 1; i < inputs.length; i+=2){
    ...
    parsedinputs.push(leftArray);
    parsedinputs.push(rightArray);
}
// pushed because we can compare by reference in sort function
parsedinputs.push(divider1);
parsedinputs.push(divider2);
console.log(parsedinputs.sort((a,b) => compare(a,b)? -1 : 1)
            .reduce((prod,elem,index) => {
                if(elem == divider1 || elem == divider2){
                    return prod * (index+1)
                }
                else{
                    return prod;
