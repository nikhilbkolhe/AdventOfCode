...
class Monkey {
    ...
    _getBored(currentInterest){
        // Monkeys no longer get bored as you are alway worried they might damage your items
        return Math.floor(currentInterest/1);
    }
    ...
    inspectItems(){
        ...
        itemCopy.forEach((item) => {
                            this._inspectNext(item);
                            this.inspectioncount++;
                        });
                    }
    }

    //TODO: learn about chinese remainder theorem
    _inspectNext(item){
        let newInterest = this.operation(item);        
        newInterest = newInterest % (5*7*13*11*3*2*17*19); // THIS IS A MODULO TRICK TO KEEP LEVELS MANAGABLE. THIS WILL REDUCE A BIG NUMBER TO A SMALL NUMBER
                                                             // without affecting any monkeys descision
